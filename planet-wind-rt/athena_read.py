    ...
    with h5py.File(filename, 'r') as f:
        ...
        for block_num in range(num_blocks):
            ...
            # Restrict fine data
                ...
                # Calculate scale
                s = int(2 ** (block_level - level) )

                # Calculate destination indices, without selection
                il_d = int(block_location[0] * block_size[0] / s) if nx1 > 1 else 0
                jl_d = int(block_location[1] * block_size[1] / s) if nx2 > 1 else 0
                kl_d = int(block_location[2] * block_size[2] / s) if nx3 > 1 else 0
                iu_d = int(il_d + block_size[0] / s) if nx1 > 1 else 1
                ju_d = int(jl_d + block_size[1] / s) if nx2 > 1 else 1
                ku_d = int(kl_d + block_size[2] / s) if nx3 > 1 else 1
                ...
                #print("indicies:",il_s, iu_s, jl_s, ju_s, kl_s, ku_s) 
                   
                # Apply subsampling
                if subsample:
                    # Calculate fine-level offsets (nearest cell at or below center)
                    o1 = int(s/2) - 1 if nx1 > 1 else 0
                    o2 = int(s/2) - 1 if nx2 > 1 else 0
                    o3 = int(s/2) - 1 if nx3 > 1 else 0
                            ...
                            for i_s, i_d in zip(i_s_vals, i_d_vals):
                                ...
                                for q, dataset, index in zip(quantities,
                                    ...
                                    data[q][k_d, j_d, i_d] += vol * f[dataset][index,
                                                                               ...
                                                                               k_s, j_s,
                                                                               i_s]
                    loc1 = int( (nx1 > 1) * block_location[0] / s )
                    loc2 = int( (nx2 > 1) * block_location[1] / s )
                    loc3 = int( (nx3 > 1) * block_location[2] / s )
